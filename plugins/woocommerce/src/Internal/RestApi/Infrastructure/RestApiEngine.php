...
class RestApiEngine {
			...
			function () {
				...
				register_rest_route(
					...
					array(
						...
						'callback'            => fn( \WP_REST_Request $request ) =>
							static::safe_invoke( 'Automattic\WooCommerce\Internal\RestApi\Controller\BarController', 'patch_rofl', $request, '', true ),
				...
				register_rest_route(
					...
					array(
						'methods'             => 'GET',
						'callback'            => fn( \WP_REST_Request $request ) =>
							static::safe_invoke( 'Automattic\WooCommerce\Internal\RestApi\Controllers\FooController', 'get_fizz', $request, 'treintaicuatror,foozer', true ),
				...
				register_rest_route(
					...
					array(
						'methods'             => 'POST,PATCH',
						'callback'            => fn( \WP_REST_Request $request ) =>
							static::safe_invoke( 'Automattic\WooCommerce\Internal\RestApi\Controllers\FooController', 'post_fizz', $request, 'administratorx,pepe', true ),
						'permission_callback' => fn() => get_current_user_id() !== 0,
					)
				);
				...
				foreach ( $additional_endpoints as $additional_endpoint_info ) {
					...
					foreach ( $additional_endpoint_info['endpoints'] as $endpoint ) {
						$allowed_roles          = $endpoint['allowed_roles'] ?? null;
	 ...
	 * @param array|null       $allowed_roles User roles allowed to process the request.
